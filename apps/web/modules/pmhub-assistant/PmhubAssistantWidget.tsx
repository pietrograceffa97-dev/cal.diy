 ...
 * panel. There are no tabs — the assistant reads what you type and routes it
 * to the right flow: walk a **test case**, report a **bug** (reproduce →
 * investigate → draft the fix ticket), ask **why** something behaves the way
 * it does (grounded in code / Confluence / Jira), or leave **feedback** (lands
 * on PM Hub's Auto-improve board). The three capabilities surface as suggested
 * prompts so a first-time user understands the surface.
 *
 * Built self-contained: its own scoped CSS variable system (`.pmha-root`) that
 * flips on cal.diy's `.dark` class, so it reads native in both themes while
 * staying pixel-faithful to the approved prototype. The one brand signature is
 * the orb's iridescent gradient, threaded into the panel's top accent.
 *
 * The chat responses below are the approved interaction shell; each `respond`
 * branch is the seam where the real PM Hub `/api/embed/*` round-trip (via the
 * same-origin `/api/pmhub-assistant/*` proxy) lands in Phases 1–3.
 */
...
/** Compact element context attached to a turn (selector + its background swatch). */
type ElChip = { selector: string; color: string };

type ChatMsg =
  | { id: number; role: "user"; text: string; el: ElChip | null }
  | { id: number; role: "bot"; intent: Intent; el: ElChip | null };
...
const SCENARIO = {
  title: "Create a 30-min event type",
  steps: [
    "Click “+ New” on the Event Types page",
    "Enter a title and confirm the URL slug auto-fills",
...
export default function PmhubAssistantWidget({ enabled, projectId }: Props) {
  ...
  const [pendingEl, setPendingEl] = useState<ElChip | null>(null);

  const idRef = useRef(0);
  const chatRef = useRef<HTMLDivElement>(null);
  const inputRef = useRef<HTMLTextAreaElement>(null);

  // Don't render on top of the PM Hub preview overlay (it owns the page then).
  useEffect(() => {
  ...
  // Keep the thread pinned to the latest turn (scrollTop, not scrollIntoView —
  // memory reference_chat_autoscroll: scrollIntoView drags the whole page).
  useEffect(() => {
    scrollToBottom();
  ...
  function pushBot(intent: Intent, el: ElChip | null) {
    setMsgs((m) => [...m, { id: nextId(), role: "bot", intent, el }]);
  }
  ...
  function sendText(text: string) {
    ...
    // Phase 1–3 seam: replace this simulated turn with a proxy round-trip to
    // PM Hub (`/api/pmhub-assistant/<flow>` → `/api/embed/*`).
    window.setTimeout(() => {
      setTyping(false);
      pushBot(intent, el);
    }, 900);
  ...
  function startPick() {
    ...
    startElementPick((el: PickedElement | null) => {
      setPicking(false);
      if (el) {
        setPendingEl({ selector: el.selector, color: el.computedStyles["background-color"] ?? "transparent" });
        openPanel();
      } else {
        // Cancelled — reopen the panel where it was.
        setOpen(true);
      }
    });
  }
  ...
  return (
    <div
      className="pmha-root"
      data-testid="pmhub-assistant.root"
                ...
                <div key={m.id} className="pmha-msg pmha-user">
                  {m.el && (
                    <div className="pmha-elchip" style={{ marginBottom: 6 }}>
                      <span className="pmha-sw" style={{ background: m.el.color }} />
                      {m.el.selector}
                    </div>
                  )}
            ...
            {pendingEl && (
              <div className="pmha-attached">
                <div className="pmha-elchip">
                  <span className="pmha-sw" style={{ background: pendingEl.color }} />
                  {pendingEl.selector}
                </div>
                <button
                  type="button"
...
function BotMessage({
  ...
  return (
    <div className="pmha-msg pmha-bot">
      ...
      {intent === "testrun" && <TestRunner scrollToBottom={scrollToBottom} />}

      {intent === "bug" && (
        <div className="pmha-bub">
          I reproduced it{ref}. The control renders but its click handler never fires the create-modal — a
          wiring gap, not styling.
          <ProposalCard scrollToBottom={scrollToBottom} />
        </div>
      )}

      {intent === "ask" && (
        <div className="pmha-bub">
          That element{ref} uses cal.diy’s <code>bg-default</code> surface (<code>rgb(255,255,255)</code>) with{" "}
          <code>text-emphasis</code> text — styled as a <b>secondary</b> action in <code>EventTypeList.tsx</code>
          . The iridescent brand fill is reserved for the primary <code>+ New</code> button, so the two never
          compete.
          <div className="pmha-pcard">
            <div className="pmha-lbl">
              <IconSpark size={11} /> Grounded in
            </div>
            <ul>
              <li>
                <code>apps/web/.../EventTypeList.tsx</code> · GitHub
              </li>
              <li>“Booking surface — button hierarchy” · Confluence</li>
            </ul>
          </div>
        </div>
      )}
      ...
      {intent === "feedback" && (
        <div className="pmha-bub">
          Got it — logged as feedback{ref}.
          <div className="pmha-ok">
            <IconCheck size={15} /> Captured to the Auto-improve board · /event-types · booking-flow-v2
          </div>
        </div>
      )}
    </div>
...
function ProposalCard({ scrollToBottom }: { scrollToBottom: () => void }) {
  ...
  return (
    <div className="pmha-pcard">
      ...
      <h4>Fix: “New” button doesn’t open the create-event modal</h4>
      <ul>
        <li>
          Clicking <code>+ New</code> on /event-types does nothing
        </li>
        <li>
          Wire onClick to <code>openCreateEventTypeModal()</code>
        </li>
        <li>Modal opens; a new event type can be created</li>
      </ul>
      {created ? (
        <div className="pmha-ok">
          <IconCheck size={15} /> Created <b>SCRUM-142</b> under the Booking Flow epic
        </div>
      ) : (
        <div className="pmha-act">
          <button
            type="button"
            className="pmha-btn pmha-prim"
            onClick={() => {
              setCreated(true);
              scrollToBottom();
            }}>
          ...
          <button type="button" className="pmha-btn">
            Edit first
          </button>
        </div>
      )}
...
function TestRunner({ scrollToBottom }: { scrollToBottom: () => void }) {
  ...
  const [phase, setPhase] = useState<"idle" | "investigating" | "proposed">("idle");
  ...
  return (
    <div className="pmha-run">
      ...
      {SCENARIO.steps.map((s, i) => {
        ...
        return (
          <div key={i}>
            <div className={`pmha-step${isPassed ? " pmha-passed" : ""}${flagIdx === i ? " pmha-failed" : ""}`}>
              <button
                type="button"
              ...
              <button type="button" className="pmha-flag" onClick={() => flag(i)}>
                <IconFlag size={12} /> Flag
              </button>
            </div>

            {flagIdx === i && (
              <div className="pmha-triage">
                {phase === "idle" && (
                  <>
                    <textarea
                      className="pmha-triage-ta"
                      placeholder="What went wrong on this step?"
                      value={triageText}
                      onChange={(e) => setTriageText(e.target.value)}
                      autoFocus
                    />
                    <div className="pmha-act">
                      <button type="button" className="pmha-btn pmha-prim" onClick={investigate}>
                        Investigate
                      </button>
                      <button type="button" className="pmha-btn" onClick={() => setFlagIdx(null)}>
                        Cancel
                      </button>
                    </div>
                  </>
                ...
                {phase === "proposed" && (
                  <>
                    <div className="pmha-triage-lead">I reproduced the failure and traced the cause.</div>
                    <ProposalCard scrollToBottom={scrollToBottom} />
                  </>
                )}
              </div>
      ...
      {allDone && (
        <div className="pmha-run-done">
          <IconCheck size={15} /> Scenario passed — logged to the test plan
        </div>
      )}
    </div>
  );
...
function IconSpark({ size = 16 }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden>
...
/**
 * Scoped style sheet. Everything is prefixed `pmha-` so nothing leaks into (or
 * gets clobbered by) cal.diy's globals. The CSS-variable palette lives on
 * `.pmha-root` and is overridden under `.dark` (cal.diy's theme class on
 * <html>), so the widget flips light/dark natively. Geist + JetBrains Mono are
 * loaded scoped to the widget; the gradient and Pac-Man chomp are the brand
 * signature.
 */
function WidgetStyles() {
  return (
    <style>{`
      ...
      .pmha-step-txt{flex:1;font-size:12.5px;color:var(--def);line-height:1.45;min-width:0}
      ...
      .pmha-flag:hover{color:var(--red);border-color:var(--red)}
      ...
      .pmha-triage-lead{font-size:12px;color:var(--def);line-height:1.5;margin-bottom:2px}
      ...
      .pmha-ok{display:flex;align-items:center;gap:8px;font-size:11.5px;color:var(--green);font-weight:600;margin-top:9px}
